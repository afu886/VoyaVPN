...
const defaultLibboxXCFramework = resolve(
  repoRoot,
  "apps",
  "desktop",
  "src-tauri",
...
function hasMacosLibboxSlice(root) {
  // XCFramework slices are immediate children named like macos-arm64_x86_64;
  // matching the full path would be vacuous because the default location
  // already contains "macos".
  for (const entry of readdirSync(root, { withFileTypes: true })) {
    if (!entry.isDirectory() || !entry.name.toLowerCase().startsWith("macos")) {
      continue;
    }
    if (existsSync(join(root, entry.name, "Libbox.framework"))) {
      return true;
    }
  }
  return false;
...
function checkLibbox() {
  if (!existsSync(libboxXCFramework)) {
    fail(`Libbox.xcframework is missing: ${libboxXCFramework}. Build it with pnpm native:macos:libbox.`);
    return;
  }
  if (!hasMacosLibboxSlice(libboxXCFramework)) {
    fail(`Libbox.xcframework has no macOS Libbox.framework slice: ${libboxXCFramework}. Rebuild it with pnpm native:macos:libbox.`);
    return;
  }
  ok(`Libbox.xcframework: ${libboxXCFramework}`);
}
